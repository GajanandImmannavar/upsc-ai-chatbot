...
# def related_results(query, results):
...
# def best_match(user_input, results):
#     matches = difflib.get_close_matches(user_input, results, n=1, cutoff=0.4)
#     return matches[0] if matches else None

# # ------------------ STREAMLIT UI ------------------

# st.set_page_config(page_title="UPSC Wikipedia Chatbot 🇮🇳", layout="wide")

# st.title("📘 UPSC Wikipedia Chatbot 🇮🇳")
# st.write("Ask about any UPSC-related topic (even if spelling is wrong)")
...
#     "Enter your question:",
#     placeholder="e.g., Rayanna (it will suggest Sangoli Rayanna)"
# )
...
#             suggestion = wikipedia.suggest(question)
#             if suggestion and suggestion not in results:
#                 results.insert(0, suggestion)

#             related = related_results(question, results)
#             if not related:
#                 related = results

#             match = best_match(question, related)
#             if match:
#                 related.remove(match)
#                 related.insert(0, match)

#             st.session_state.results = related[:10]
...
#         format_func=lambda x: f"{x+1}. {st.session_state.results[x]}"
#     )

#     if st.button("Get Details"):
#         try:
#             selected_title = st.session_state.results[choice]
#             page = wikipedia.page(selected_title, auto_suggest=False)

#             content = clean_content(page.content)
#             sections = get_sections(content, max_sections=4)
...
#             st.markdown("""
#             - **Prelims:** Factual understanding  
#             - **Mains:** GS + Essay answers  
#             - **Interview:** Conceptual clarity  
#             """)

#         except Exception:
#             st.error("Invalid choice or page could not be loaded.")
import streamlit as st
import wikipedia
import difflib
import re
